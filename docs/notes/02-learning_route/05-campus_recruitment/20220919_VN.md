...
layout: post
category: algorithm
title: Xuất sắc giành Offer Thực tập Backend Douyin (ByteDance)
tagline: by A Tú
tags:
    - Nguyên tác
    - Máy tính
    ...
    - A Tú
excerpt: Xuất sắc giành Offer Thực tập Backend Douyin (ByteDance)
comment: false
---

<h1 align="center">Xuất sắc giành Offer Thực tập Backend Douyin (ByteDance)</h1>
...
> Tác giả: A Tú (阿秀)
>
> Liên kết bài viết gốc: [https://mp.weixin.qq.com/s/Agc8CbzwZqfp4wPkQKacGg](https://mp.weixin.qq.com/s/Agc8CbzwZqfp4wPkQKacGg)

![](http://oss.interviewguide.cn/img/202210221634308.png)

Kinh nghiệm trúng tuyển vị trí **Backend Douyin (TikTok Core)** - một trong những bộ phận có lượng truy cập và yêu cầu kỹ thuật khắt khe nhất tại ByteDance:

### Bộ câu hỏi 3 vòng phỏng vấn thực tế tại Douyin Backend

#### Vòng 1 (60 phút):
- So sánh phương thức `PUT` và `POST` trong chuẩn RESTful API.
- 3 tính chất OOP trong C++ (Đóng gói, Kế thừa, Đa hình), so sánh `std::vector` vs `std::list`.
- Giao thức HTTP/3: Cơ chế dựa trên UDP và giao thức QUIC (0-RTT Handshake, giải quyết Head-of-Line Blocking).
- **Thuật toán**: Tổng 3 số (3Sum - LeetCode 15).

#### Vòng 2 (60 phút):
- Nguyên lý định tuyến thông điệp ngầm định trong framework Backend.
- Biện minh quyết định Tech Stack: Vì sao chọn PostgreSQL thay vì MySQL, vì sao chọn Django thay vì Spring Boot?
- Toàn bộ luồng dữ liệu khi trình duyệt gửi yêu cầu lấy ảnh từ máy chủ.
- So sánh Process, Thread và Coroutine (Goroutine).
- **Bài toán tình huống 1**: Thuật toán chia ngẫu nhiên $M$ tiền thành $N$ phong bao lì xì (Red Packet Algorithm), đảm bảo phân bố đều, mỗi bao tối thiểu 0.01 và là bội số của 0.01.
- **Bài toán tình huống 2**: Mê cung ma trận $M \times N$, đếm số đường đi từ ô $(0,0)$ đến $(M-1, N-1)$ (Dynamic Programming và phương pháp Tổ hợp Toán học).

#### Vòng 3 (Leader - 40 phút):
- Làm thế nào để đảm bảo chất lượng kỹ thuật (Engineering Quality Assurance) trong các dự án lớn?
- **Thiết kế kiến trúc hệ thống**: Thiết kế hệ thống cảnh báo và giám sát thời gian thực cho nền tảng Douyin (Real-time Metric Monitoring & Alerting System Architecture).
