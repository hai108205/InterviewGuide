...
layout: post
category: algorithm
title: Đỗ Hạ tầng ByteDance (ByteDance Infrastructure) - Bước ngoặt sau khi chuyển ngành từ Xây dựng
tagline: by A Tú
tags:
    - Nguyên tác
    - Máy tính
    ...
    - A Tú
excerpt: Đỗ Hạ tầng ByteDance (ByteDance Infrastructure) - Bước ngoặt sau khi chuyển ngành từ Xây dựng
comment: false
---

<h1 align="center">Đỗ Hạ tầng ByteDance (ByteDance Infrastructure) - Bước ngoặt sau khi chuyển ngành từ Xây dựng</h1>
...
> Tác giả: A Tú (阿秀)
>
> Liên kết bài viết gốc: [https://mp.weixin.qq.com/s/p_1B66vJSr2pnjwKKOVa6Q](https://mp.weixin.qq.com/s/p_1B66vJSr2pnjwKKOVa6Q)

Chia sẻ cực kỳ **hardcore** của một bạn sinh viên chuyển ngành từ Kỹ thuật Xây dựng (Civil Engineering) sang CNTT (chấp nhận học đại học 5 năm do lùi 1 khóa), tự tay thiết kế Hệ điều hành & CPU mô phỏng và đỗ **Hạ tầng cơ sở ByteDance (ByteDance Infrastructure)**:

### 1. Bí quyết phỏng vấn đột phá: Chuẩn bị Slide PPT cho dự án
- Khi giới thiệu bản thân, chủ động giới thiệu dự án nghiên cứu Hệ điều hành/CPU và ebook tự viết.
- Trình chiếu Slide PPT: Vừa giúp người phỏng vấn nắm bắt mạch tư duy, vừa chuyển thế chủ động về phía mình, chiếm phần lớn thời lượng phỏng vấn bằng chuyên môn thế mạnh thay vì bị quay cuồng trong những câu hỏi khó lường.

---

### 2. Tổng hợp kiến thức chuyên sâu trong các vòng phỏng vấn Hạ tầng:

#### C++ & Compilation / Linking:
- Tối ưu hóa giá trị trả về RVO / NRVO (Return Value Optimization).
- Tại sao không nên gọi hàm ảo `virtual` bên trong Constructor hoặc Destructor (*Effective C++*).
- Bản chất `std::shared_ptr`: Khối điều khiển Control Block và cơ chế Reference Counting.
- Định dạng tệp thực thi ELF (ELF Headers, String Table, Symbol Table, Relocation).

#### Hệ điều hành & Linux Kernel (4 trụ cột tự hiện thực trong dự án OS):
- **Memory Management**: Quản lý bộ nhớ vật lý bằng Free List; cơ chế Buddy System và Slab Allocator; 3 dạng Page Fault (Swap, Copy-on-Write, Lazy Allocation).
- **Driver Management**: Toàn bộ chu trình từ khi nhấn phím bàn phím vật lý đến khi ký tự hiển thị lên màn hình (Keyboard Interrupt -> Interrupt Service Routine -> Framebuffer/GPU Driver).
- **File System**: Cấu trúc Inode, Thao tác tệp `open/read/write/dup`, Soft Link vs Hard Link; thiết kế Journaling File System để đảm bảo Crash Consistency.
- **Process Management**: Quá trình chuyển đổi ngữ cảnh (Context Switching: đổi Stack, thay đổi Page Table, Flush TLB), chuyển đổi Kernel Stack vs User Stack (`ESP0` trong x86 TSS).

#### Kiến trúc máy tính & Thiết kế CPU (MIPS / RISC-V):
- Nguyên lý thiết kế Pipeline (Instruction Pipelining).
- Các loại xung đột trong Pipeline: Structural Hazard, Data Hazard, Control Hazard và cách xử lý (Bypassing/Forwarding, Branch Prediction).
- Phân biệt kiến trúc RISC vs CISC; cơ chế đảm bảo tính nhất quán của bộ nhớ đệm (Cache Coherence - Giao thức MESI).
