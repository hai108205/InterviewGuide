...
layout: post
category: prepare
title: Thấy dự án hay trên GitHub nhưng không biết bắt đầu học từ đâu?
tagline: by A Tú
tags:
    - Nguyên tác
    ...
    - A Tú
excerpt: Thấy dự án hay trên GitHub nhưng không biết bắt đầu học từ đâu?
comment: false
---

<h1 align="center">
Thấy dự án hay trên GitHub nhưng không biết bắt đầu học từ đâu?
</h1>
...
> Tác giả: A Tú (阿秀)
>
> Liên kết bài viết gốc: [https://mp.weixin.qq.com/s/XY3fbN-PWYCymOG8r8X7og](https://mp.weixin.qq.com/s/XY3fbN-PWYCymOG8r8X7og)

![Học hỏi từ các dự án mã nguồn mở](http://oss.interviewguide.cn/img/202206070136751.png)

Phương pháp 4 bước hiệu quả để nghiên cứu và chuyển hóa các dự án mã nguồn mở (Open Source Projects) trên GitHub thành dự án ấn tượng ghi vào CV:

---

### Quy trình 4 bước tiếp cận dự án GitHub:

1. **Bước 1: Chạy dự án trên Local trước tiên (Make it Run!)**:
   - Đọc kỹ `README.md`, cài đặt đầy đủ Dependency (Dependencies/Packages), cấu hình cơ sở dữ liệu và biến môi trường (`.env`).
   - Nếu gặp lỗi khởi động, tra cứu mục `Issues` của repo để xem cách cộng đồng khắc phục.
2. **Bước 2: Tìm điểm vào (Entry Point) & Phân tách module cốt lõi**:
   - Xác định luồng dữ liệu (Data Flow): Điểm bắt đầu nhận Request $\rightarrow$ Middleware $\rightarrow$ Controller $\rightarrow$ Service xử lý nghiệp vụ $\rightarrow$ DAO/Repository lưu CSDL.
3. **Bước 3: Tự tay viết lại từng module (Hands-on Re-implementation)**:
   - *"Tay làm được mới là thật sự hiểu, não thấy hiểu chưa chắc đã hiểu."*
   - Tự tay gõ lại module then chốt, so sánh kết quả với module gốc của tác giả. Sau đó tiến hành tùy biến, mở rộng thêm tính năng mới (ví dụ: thêm cơ chế Logging, JWT Authentication, Connection Pool hoặc Rate Limiting).
4. **Bước 4: Mở rộng từ điểm sang diện (Expand System Scope)**:
   - Kết nối module vừa hoàn thành với các module lân cận để có cái nhìn toàn cục về kiến trúc hệ thống.

![Nghiên cứu dự án](http://oss.interviewguide.cn/img/202206070132554.png)
![Thực hành gõ code](http://oss.interviewguide.cn/img/202206070128656.png)
